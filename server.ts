...
// Curated flight search results tailored by origin country and hub
function getFlightsForDestination(cityCode: string, origin: string = 'SFO', originCountry: string = 'United States') {
  const originCode = origin.includes('(') ? origin.split('(')[1].replace(')', '').trim() : (origin || 'SFO').trim();
  const dest = cityCode === 'TYO' ? 'HND' : cityCode;

  if (originCountry.includes('Kingdom') || ['LHR', 'LGW', 'MAN', 'EDI'].includes(originCode)) {
    return [
      {
        id: 'fl-uk-1',
      ...
      },
      {
        ...
        arrivalTime: '05:35 PM (+1d)',
        departureAirport: originCode,
        arrivalAirport: dest,
        duration: '16h 40m',
        stops: 1,
        layoverInfo: '2h Dubai (DXB)',
        price: 860,
        cabinClass: 'Economy' as const,
  ...
  } else if (originCountry.includes('Australia') || ['SYD', 'MEL', 'BNE', 'PER'].includes(originCode)) {
    return [
      {
        id: 'fl-au-1',
      ...
      },
      {
        ...
        duration: '11h 20m',
        stops: 1,
        layoverInfo: '1h 30m Changi (SIN)',
        price: 810,
        cabinClass: 'Economy' as const,
        baggage: '2 x 25kg checked bags included',
        carbonKg: 395,
        aircraft: 'Boeing 777-300ER',
      },
    ];
  } else if (originCountry.includes('Singapore') || ['SIN'].includes(originCode)) {
    return [
      {
        id: 'fl-sg-1',
        airline: 'Singapore Airlines',
      ...
      },
      {
        ...
        price: 450,
        cabinClass: 'Economy' as const,
        baggage: '1 x 20kg checked bag',
        carbonKg: 290,
        aircraft: 'Boeing 787-8 Dreamliner',
      },
    ];
  } else if (originCountry.includes('Canada') || ['YVR', 'YYZ', 'YUL', 'YYC'].includes(originCode)) {
    return [
      {
        id: 'fl-ca-1',
        airline: 'Air Canada',
  ...
  app.post('/api/task', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (task === 'search_flights') {
        ...
        const originCountry = payload?.originCountry || 'United States';
        const flights = getFlightsForDestination(destCode, origin, originCountry);
        
        recordMcpLog('flight-search-mcp', 'query_inventory_pricing', Date.now() - startTime, 'success', { destCode, origin, originCountry }, `Found ${flights.length} flights from ${origin} (${originCountry}) to ${destCode}`);
